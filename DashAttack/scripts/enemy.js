class Enemy {
    ...
    applyEffect(name, duration, tower) {
        if (this.immune.includes(name)) return;
        if (getByName(this.effects, name).length > 0){
            var tab = getByName(this.effects, name);
            var newduration = duration - tab[0].duration;
            getByName(this.effects, name)[0].duration = Math.round(duration);
            getByName(this.effects, name)[0].tower = tower;
            duration = newduration;
        }else{
            var e = createEffect(duration, effects[name], tower);
            e.onStart(this);
            this.effects.push(e);
        }
        return duration;
