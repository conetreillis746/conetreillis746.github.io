...
tower.slow = {
    ...
    onHit: function(e, damage) {
        var time = 40;
        time += (time / 120) * 0.01 * this.level;
        time = e.applyEffect('slow', time, this);
        this.lootExp(time);
    },
    ...
    upgrades: [
        {
            ...
            onHit: function(e, damage) {
                var time = 60;
                time += time * 0.01 * this.level;
                time = e.applyEffect('poison', time, this);
                // this.lootExp(time);
            }
        }
    ]
...
tower.rocket = {
    ...
    onAim(e) {
        ...
        if (!this.canFire()) return;
        this.resetCooldown();
        projectiles.push(new Missile(this.pos.x, this.pos.y, e, this));
        projectiles.at(-1).lifetime = 60 + this.level;
    ...
    upgrades: [
        {
            ...
            onAim(e) {
                ...
                if (!this.canFire()) return;
                this.resetCooldown();
                var m = new Missile(this.pos.x, this.pos.y, e, this);
                m.color = [65, 131, 215];
...
tower.tesla = {
    ...
    onAim(e) {
        ...
        var last = e;
        var targets = [];
        var dmg = round(this.getAttackDamage());
        ...
        while (dmg > 1) {
            ...
            { // calcul length between actual enemie and next enemie
                var tilesize = (ts / 2);
                var tab = {
                    'x': (last.pos.x - next.pos.x) / tilesize,
                    'y': (last.pos.y - next.pos.y) / tilesize,
                };
                if(tab.x < 0) tab.x = tab.x * -1;
                if(tab.y < 0) tab.y = tab.y * -1;
                var length = Math.sqrt(Math.pow(tab.x,2) + Math.pow(tab.y,2));
            }
            if(length > this.range){ // can reach the next enemie
                break;
            }else{
...
tower.random = {
    ...
    getAttackDamage(){
        var damage = random(this.damageMin + (this.level - 1), (this.damageMax + this.level - 1) * this.level);
        return damage;
    },
    // Returns damage range
    getDamage() {
        var min = this.damageMin + (this.level - 1);
        var max = (this.damageMax + this.level - 1) * (1 + (this.level - 1) * 0.1);
        min = Number.parseFloat(min).toFixed(min>1 || min === 0 ? 2 : 3);
        max = Number.parseFloat(max).toFixed(max>1 || min === 0 ? 2 : 3);
        return rangeText(min , max);
    },
    getCooldown() {
        return (this.cooldownMin + this.cooldownMax / this.level) / 120;
    },
    resetCooldown() {
        var cooldown = round(random(this.cooldownMin, this.cooldownMax / this.level));
        this.cd = cooldown;
        this.maxcd = this.cd;
    },
    lootExp(damage){
        this.exp = Number.parseFloat(Number.parseFloat(this.exp) + damage * 5) + this.cost;
